...
CONCUERROR_RUN := $(CONCUERROR) \
	-x code -x code_server -x error_handler --assertions_only \
	--pa $(BUILD_DIR)/concuerror+test/checkouts/snabbkaffe/ebin \
  --pa $(BUILD_DIR)/concuerror+test/lib/anvl_core/ebin
...
concuerror = $(CONCUERROR_RUN) -f $(BUILD_DIR)/concuerror+test/lib/$(1)/test/concuerror_tests.beam -t $(2) || \
	{ cat concuerror_report.txt; exit 1; }

.PHONY: concuerror_test
concuerror_test: $(CONCUERROR)
	rebar3 as concuerror eunit
	$(call concuerror,anvl_make,dependency_test)
